...
class hadoop_cluster_node (
  ...
  $matrixnorm_real_users = [ 'jenkins', 'testuser', 'hudson' ],
  $cluster_components = ["all"]
) {

  user { $matrixnorm_real_users:
    ensure     => present,
    system     => false,
    managehome => true,
  }

  $hadoop_head_node = hiera("matrixnorm::hadoop_head_node")
  $standby_head_node = hiera("matrixnorm::standby_head_node", "")
  $hadoop_gateway_node = hiera("matrixnorm::hadoop_gateway_node", $hadoop_head_node)
...
class node_with_roles ($roles = hiera("matrixnorm::roles")) inherits hadoop_cluster_node {
